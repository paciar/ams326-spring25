...
"""
Buffon Disc Problem
- Randomly generate points (within a certain range) using a uniform distribution to serve as the center of each disc
...
- Simulate for all diameters d and keep counters for how many lines were crossed (using a dict) -- note that if a line crosses at least 2 lines, it also crosses at least 1 line (the probabilities are calculated for "at least" 1, 2, 3, 4... lines in a cumulative fashion)
- Calculate probabilities using (number of times at least n lines were crossed) / (total discs used) for all n = 1, 2, 3, 4 lines
"""
...
def buffon_disc(d, n):
    ...
    # crossings[i] = how many times at least (i+1) lines were crossed
    crossings = [0, 0, 0, 0]
    for center in disc_centers:
        ...
        center_x = center[0]
        # Wrapped with max(0, ...) to guard against possible floating-point errors with floor/ceil
        num_crossings = max(0, math.floor(center_x + r) - math.ceil(center_x - r) + 1)
        # We want to update the number of crossings "cumulatively" -- if a disc crossed at least 3 lines, it also crossed at least 2 lines, and at least 1 line
...
def main():
    ...
    # Dict to map diameter to probabilities
    probs = {}
    for d in diameters:
    ...
    ax.set_xticks(x + width * 2, diameter_labels)
    ax.set_ylim(0, 1.1)
    custom_legend = [
